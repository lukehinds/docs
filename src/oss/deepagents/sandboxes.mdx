...
Agents generate code, interact with filesystems, and run shell commands. Because you cannot predict what an agent might do, it is important that its environment is isolated so it cannot access credentials, files, or the network. Sandboxes provide this isolation by creating a boundary between the agent's execution environment and your host system.
...
<Note>
**If you are looking for LangSmith sandboxes:** LangSmith provides first-party managed sandboxes you can use directly from the LangSmith UI or SDK without a third-party account required. For managed sandbox resources, snapshots, service URLs, and the auth proxy, refer to [LangSmith Sandboxes](/langsmith/sandboxes).
</Note>

<Note>
**If you are looking for nono sandboxes:** nono provides local, OS-enforced sandboxing for Python Deep Agents with no remote provider account, container, or VM required. Use it when you want the agent to run shell commands on the same machine while restricting filesystem access, network access, and credential exposure. See the [nono provider page](/oss/python/integrations/providers/nono) for installation and configuration.
</Note>
...
Do not see your provider? You can implement your own sandbox backend. See [Contributing a sandbox integration](/oss/contributing/integrations-langchain).
...
<Tip>
**TTL for chat applications.** When users can re-engage after idle time, you often do not know if or when they return. Configure a time-to-live (TTL) on the sandbox, for example, TTL to archive or TTL to delete, so the provider automatically cleans up idle sandboxes. Many sandbox providers support this.
</Tip>
...
<Tabs>
    ...
    <Tab title="nono">
        ...
        With nono, each conversation gets its own working directory. There is no remote state to manage, create the backend with a thread-specific path:
        ...
        # Each thread gets its own isolated working directory.
...
There are three architecture patterns for integrating agents with sandboxes, based on where the agent runs.

### Run the agent inside a managed sandbox

This pattern applies to managed sandbox services that provide remote infrastructure. The agent runs inside the sandbox and you communicate with it over the network. You build a Docker or VM image with your agent framework pre-installed, run it inside the sandbox, and connect from outside to send messages.
...
To run an agent in a managed sandbox, build an image and install deepagents on it.
...
<Note>
Embedded sandbox providers like nono do not use this pattern. They enforce isolation on the same machine the agent runs on, so there is no separate remote environment to run an agent inside. See [Call the sandbox as a tool](#call-the-sandbox-as-a-tool) instead.
</Note>

### Call the sandbox as a tool

The agent runs on your machine or server. When it needs to execute code, it calls sandbox tools (such as `execute`, `read_file`, or `write_file`) which route over the network to a remote sandbox managed by a service like Daytona, Modal, Runloop, or E2B.
...
- ✅ Cleaner separation between agent state and execution.
    ...
    - Sandbox failures do not lose agent state.
...
### Use embedded sandboxing

Embedded providers like nono run on the same machine as the agent. Each `execute()` call forks a child process with kernel-enforced restrictions using native Linux and MacOS mechanisms. There is no remote infrastructure: the sandbox is linked with the agent's runtime. This works anywhere the agent runs: laptops, cloud VMs, Kubernetes, CI.

Benefits:

- ✅ Zero latency: no network round-trip.
- ✅ No account, API keys, or external service dependency required.
- ✅ No cleanup required: processes are ephemeral forks.

Trade-offs:

- 🔴 Bound to the host machine's resources.
...
The examples in this doc use the sandbox-as-tool and embedded sandboxing patterns.
Choose the agent-in-sandbox pattern when your managed service handles the communication layer and you want production to mirror local development.
Choose the sandbox-as-tool pattern when you need to iterate quickly on agent logic, keep API keys outside the sandbox, or prefer cleaner separation of concerns. This pattern works with both managed services and embedded providers like nono.
...
- **Network exfiltration**: Unless network access is blocked, a context-injected agent can send data out of the sandbox over HTTP or DNS. Some providers support blocking network access (e.g., `blockNetwork: true` on Modal). Embedded providers like nono can block outbound network access by default and, when network access is needed, route requests through a proxy with host allowlisting, L7 endpoint filtering, and SSRF protection.
...
There are two distinct ways files move in and out of a sandbox, and it is important to understand when to use each:
...
<Tabs>
    ...
    <Tab title="nono">
        ...
        backend.upload_files(
            [
                ("/tmp/agent-workspace/src/index.py", b"print('Hello')\n"),
                ("/tmp/agent-workspace/pyproject.toml", b"[project]\nname = 'my-app'\n"),
            ]
        )
        ```
    </Tab>
...
<Tabs>
    ...
    <Tab title="nono">
        ...
        results = backend.download_files(
            ["/tmp/agent-workspace/src/index.py", "/tmp/agent-workspace/pyproject.toml"]
        )
        for result in results:
            if result.content is not None:
                print(f"{result.path}: {result.content.decode()}")
...
Managed sandbox services isolate code execution from your host system, but they do not protect against **context injection**. An attacker who controls part of the agent's input can instruct it to read files, run commands, or exfiltrate data from within the sandbox. This makes credentials inside managed sandboxes especially dangerous.

<Warning>
**Never put secrets inside a managed sandbox.** API keys, tokens, database credentials, and other secrets injected into a managed sandbox (via environment variables, mounted files, or the `secrets` option) can be read and exfiltrated by a context-injected agent. This applies even to short-lived or scoped credentials: if an agent can access them, so can an attacker.
</Warning>

#### Handling secrets safely

If your agent needs to call authenticated APIs or access protected resources from a managed sandbox, there are two approaches:

1. **Keep secrets in tools outside the sandbox.** Define tools that run in your host environment (not inside the sandbox) and handle authentication there. The agent calls these tools by name, but never sees the credentials.

2. **Use a third-party network proxy.** Some managed sandbox providers support proxies that attach credentials to outgoing requests. The agent makes plain requests to a URL, and the proxy injects the `Authorization` header before forwarding. This approach is not yet widely available across providers.

<Warning>
If you must inject secrets directly into a managed sandbox (not recommended), take these precautions:
...
Even with these safeguards, this remains an unsafe workaround. A sufficiently creative context injection attack can bypass output filtering and HITL review.
</Warning>

### Credential protection with embedded sandboxing

Embedded providers like nono are designed to handle credential protection natively. nono blocks all outbound network access by default and includes a built-in credential-injecting proxy that keeps real secrets outside the sandbox entirely.

The proxy intercepts outgoing HTTP requests from the sandbox and injects credentials (API keys, tokens) before forwarding them to the upstream service. The agent only sees a session-scoped phantom token; real credentials never enter the sandbox. The proxy also provides host allowlisting, L7 endpoint filtering, and SSRF protection (cloud metadata endpoints are hard-denied). See the [nono provider page](/oss/python/integrations/providers/nono) for configuration details.
