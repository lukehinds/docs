...
<Tip>
    ...
    **Deploy users:** Define subagents as directories under `subagents/` with their own `deepagents.toml` and `AGENTS.md`. The bundler auto-discovers them. See [Deploy subagents](/oss/python/deepagents/deploy#subagents) for the full configuration reference.
</Tip>
